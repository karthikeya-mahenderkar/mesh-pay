...
class IdempotencyConcurrencyTest {
    ...
    @Test
    void incorrectPinIsRejected() throws Exception {
        // alice@demo's PIN is "1234" (seeded in DemoService)
        MeshPacket packet = demoService.createPacket(
                "alice@demo", "bob@demo", new BigDecimal("50.00"), "9999", 5);

        BridgeIngestionService.IngestResult r = bridge.ingest(packet, "bridge-x", 1);

        assertEquals("SETTLED", r.outcome(), "The ingestion itself succeeded");
        assertNotNull(r.transactionId());
        
        var tx = transactions.findById(r.transactionId()).orElseThrow();
        assertEquals(com.demo.upimesh.model.Transaction.Status.REJECTED, tx.getStatus());
    }
