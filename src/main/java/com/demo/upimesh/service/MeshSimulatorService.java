...
public class MeshSimulatorService {
    ...
    /**
     * Automated Background Gossip.
     * Runs every 5 seconds to simulate ongoing BLE mesh activity.
     * It only logs if a transfer actually happened to reduce console noise.
     */
    @org.springframework.scheduling.annotation.Scheduled(fixedRate = 5000)
    public void scheduledGossip() {
        GossipResult result = gossipOnce();
        if (result.transfers() > 0) {
            log.info("[AUTO-GOSSIP] Background mesh propagation: {} hops", result.transfers());
        }
    }
